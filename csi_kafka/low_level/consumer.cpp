...
namespace csi
{
    ...
    namespace kafka
    {
        lowlevel_consumer2::lowlevel_consumer2(boost::asio::io_service& io_service, const std::string& topic, int32_t partition, int32_t rx_timeout) :
            _ios(io_service),
            _client(io_service),
            _topic(topic),
        ...
        lowlevel_consumer2::~lowlevel_consumer2()
        {
            _client.close();
            _metrics_timer.cancel();
        }

        void lowlevel_consumer2::handle_metrics_timer(const boost::system::error_code& ec)
        {
            if (ec)
                return;
        ...
        void lowlevel_consumer2::set_offset_async(int64_t start_time, set_offset_callback cb)
        {
        ...
        rpc_result<void> lowlevel_consumer2::set_offset(int64_t start_time)
        {
            std::promise<rpc_result<void>> p;
            std::future<rpc_result<void>>  f = p.get_future();
        ...
        void lowlevel_consumer2::_try_fetch()
        {
        ...
        void lowlevel_consumer2::stream_async(datastream_callback cb)
        {
            _cb = cb;
            _ios.post([this](){_try_fetch(); });
        }

        void lowlevel_consumer2::get_metadata_async(get_metadata_callback cb)
        {
            _client.get_metadata_async({ _topic }, 0, cb);
        }
    } // kafka
